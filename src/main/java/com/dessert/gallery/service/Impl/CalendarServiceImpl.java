...
public class CalendarServiceImpl implements CalendarService {
    ...
    public CalendarResponseDto getCalendarByStore(Long storeId, int year, int month) {
        ...
        LocalDateTime endOfMonth = startOfMonth.withDayOfMonth(startOfMonth.getMonth()
                .length(startOfMonth.toLocalDate().isLeapYear())).with(LocalTime.MAX);
    ...
    public CalendarOwnerResponseDto getOwnerCalendar(int year, int month, HttpServletRequest request) {
        ...
        if (user == null)
            throw new NotFoundException("존재하지 않는 유저", NOT_FOUND_EXCEPTION);
    ...
    private CalendarResponseDto toCalendarDto(int year, int month, List<Schedule> scheduleList) {
        Map<Integer, ScheduleResponseDto> map = new HashMap<>();

        for (Schedule schedule : scheduleList) {
            int key = schedule.getDateTime().getDayOfMonth(); // 날짜에 따라 스케줄 반환 객체 관리
            ScheduleResponseDto dto = map.getOrDefault(key, new ScheduleResponseDto(schedule));

            if (schedule.getType() == ScheduleType.HOLIDAY)
                dto.setHoliday();
            if (schedule.getType() == ScheduleType.EVENT)
                dto.setEvent();
            if (!dto.isReservation() && schedule.getType() == ScheduleType.RESERVATION)
                dto.setReservation();

            map.put(key, dto);
        }

        List<ScheduleResponseDto> scheduleResponseDtoList = new ArrayList<>(map.values());
